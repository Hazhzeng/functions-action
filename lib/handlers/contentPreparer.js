...
class ContentPreparer {
    ...
    getPomXmlSourceLocation(packagePath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!fs_1.existsSync(pomXmlPath)) {
                utils_1.Logger.Warn(`The file ${pomXmlPath} does not exist. Please ensure your publish-profile setting points to a folder containing host.json.`);
                utils_1.Logger.Warn(`Using ${packagePath} as source folder for packaging`);
                return packagePath;
            }
            ...
            try {
                ...
                utils_1.Logger.Warn(`The file ${pomXmlPath} does not have valid content, using ${packagePath} as source folder for packaging`);
                return packagePath;
            }
            ...
            if (!pomXmlResult) {
                utils_1.Logger.Warn(`The xml file ${pomXmlPath} is invalid, using ${packagePath} as source folder for packaging`);
                return packagePath;
            }
            ...
            try {
                ...
                utils_1.Logger.Warn(`Cannot find functionAppName section in pom.xml, using ${packagePath} as source folder for packaging`);
                return packagePath;
            }
            const pomPackagePath = path_1.resolve(packagePath, 'target', 'azure-functions', functionAppName);
            utils_1.Logger.Info(`Sucessfully parsed pom.xml. Using ${pomPackagePath} as source folder for packaging`);
    ...
    derivePublishMethod(state, packageType, osType, sku, authenticationType) {
        ...
        if (authenticationType == authentication_type_1.AuthenticationType.Scm) {
            utils_1.Logger.Info('Will use api/zipdeploy to deploy (scm credential)');
        ...
        if (osType === runtime_stack_1.RuntimeStackConstant.Linux && sku === function_sku_1.FunctionSkuConstant.Consumption) {
            utils_1.Logger.Info('Will use WEBSITE_RUN_FROM_PACKAGE to deploy');
            return publish_method_1.PublishMethodConstant.WebsiteRunFromPackageDeploy;
        }
        // Rest Skus which support api/zipdeploy endpoint
        utils_1.Logger.Info('Will use api/zipdeploy to deploy (rbac authentication)');
        return publish_method_1.PublishMethodConstant.ZipDeploy;
    }
}
